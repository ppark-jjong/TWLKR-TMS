...
import { useLogger } from "../utils/LogUtils"; // 로깅 유틸리티 사용
...
const useDashboardPageController = () => {
  const logger = useLogger("DashboardPageController");
  const { user } = useAuth();
  ...
  const [typeFilter, setTypeFilter] = useState(filterState.typeFilter);
  const [departmentFilter, setDepartmentFilter] = useState(
    filterState.departmentFilter
  );
  const [warehouseFilter, setWarehouseFilter] = useState(
    filterState.warehouseFilter
  );
  const [orderNoSearch, setOrderNoSearch] = useState(filterState.orderNoSearch);
  const [filterButtonClicked, setFilterButtonClicked] = useState(false);

  // 요청 중복 방지를 위한 플래그 - useRef로 관리하여 리렌더링 방지
  const isRefreshingRef = useRef(false);
  const isSearchingRef = useRef(false);
  const isLoadingDetailRef = useRef(false);
  const filterAppliedRef = useRef(false);

  // 대시보드 데이터 로드 함수 - useCallback으로 메모이제이션
  ...
  // 삭제 핸들러 (관리자 전용) - useCallback으로 메모이제이션
  ...
  // 주문번호 검색 핸들러 - useCallback으로 메모이제이션
  ...
  // 배차 버튼 클릭 핸들러 - useCallback으로 메모이제이션
  ...
  // 모달 관련 핸들러 - useCallback으로 메모이제이션
  ...
  // 필터 관련 핸들러 - useCallback으로 메모이제이션
  ...
  // 필터 적용 트리거 - useCallback으로 메모이제이션
  const handleApplyFilters = useCallback(() => {
    setFilterButtonClicked(true);
  }, []);

  // 메모이제이션된 반환값으로 렌더링 최적화
  return useMemo(
    () => ({
      // 상태
      dashboards,
