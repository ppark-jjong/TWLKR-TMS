...
async def get_visualization_data(
    ...
    _: str = Depends(get_current_user_id),  # JWT 토큰 검증
):
    """시각화 데이터 조회 API"""
    try:
        # 날짜 범위 검증 (1개월 이내)
        ...
        if days_diff > 31:
            raise HTTPException(
                status_code=400, detail="조회 기간은 1개월을 초과할 수 없습니다"
            )
        ...
        if date_range.end_date > today:
            raise HTTPException(
                status_code=400, detail="미래 날짜는 조회할 수 없습니다"
            )
        ...
        return service.get_visualization_data(viz_type, start_datetime, end_datetime)
