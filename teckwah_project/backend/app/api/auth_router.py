...
async def login(
    login_data: UserLogin, service: AuthService = Depends(get_auth_service)
):
    """로그인 API"""
    try:
        token, user = service.authenticate_user(login_data)
        return {"token": token.dict(), "user": user.dict()}
...
@router.post("/logout")
async def logout(refresh_token: str, service: AuthService = Depends(get_auth_service)):
    """로그아웃 API"""
    success = service.logout(refresh_token)
    if not success:
        raise HTTPException(status_code=400, detail="로그아웃 처리에 실패했습니다")
