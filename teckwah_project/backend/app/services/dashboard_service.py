...
class DashboardService:
    ...
    async def get_list(self, target_date: datetime) -> List[Dict]:
        """날짜별 대시보드 목록 조회"""
        try:
            # 날짜 검증
            ...
            return await self.repository.get_list_by_date(target_date)
    ...
    async def create_dashboard(self, data: DashboardCreate, user: User) -> DashboardResponse:
        """대시보드 생성"""
        try:
    ...
    async def assign_driver(
        ...
        """기사 배차"""
        try:
            result = await self.repository.assign_driver(dashboard_ids, driver_id, driver_remark)
    ...
    async def delete_dashboards(self, dashboard_ids: List[int]) -> DashboardDeleteResponse:
        """대시보드 일괄 삭제"""
        try:
