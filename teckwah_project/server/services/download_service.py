# teckwah_project/server/services/download_service.py
import io
...
from server.repositories.download_repository import DownloadRepository
from server.utils.logger import log_info, log_error
from server.utils.datetime import format_datetime
from server.utils.constants import MESSAGES, TYPE_TEXT_MAP, STATUS_TEXT_MAP
...
class DownloadService:
    ...
    def get_dashboard_data_for_download(
        ...
        try:
            ...
            # 1. 데이터 조회
            dashboards = self.repository.get_dashboard_data_by_create_time(
                start_time, end_time
            )
            if not dashboards:
            ...
            # 2. Excel 파일 생성
            file_content, file_name = self._generate_dashboard_xlsx(
                dashboards, start_time, end_time
            )
        ...
        try:
            ...
            for row_idx, dashboard in enumerate(dashboards, 2):
                ...
                if hasattr(dashboard, "remarks") and dashboard.remarks:
                    remarks = sorted(
                        dashboard.remarks, key=lambda r: r.created_at, reverse=True
                    )
                ...
                row_data = [
                    ...
                    dashboard.sla,  # SLA
                    (
                        format_datetime(dashboard.eta) if dashboard.eta else ""
                    ),  # 예상도착시간
                    (
                        format_datetime(dashboard.create_time)
                        if dashboard.create_time
                        else ""
                    ),  # 생성시간
                    (
                        format_datetime(dashboard.depart_time)
                        if dashboard.depart_time
                        else ""
                    ),  # 출발시간
                    (
                        format_datetime(dashboard.complete_time)
