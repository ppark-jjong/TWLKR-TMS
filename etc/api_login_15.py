...
def get_route_distances(start_x, start_y, end_x, end_y, client_id, client_secret):
    ...
    for option_code, option_name in options:
        ...
        try:
            ...
            if response.status_code == 200:
                result = response.json()
                # direction15 API는 응답 JSON 구조가 아래와 같이 달라질 수 있음.
                # 케이스 1: result가 최상위에 있고, result["route"]에 옵션이 있는 경우
                if "route" in result and option_code in result["route"]:
                    distance = (
                        result["route"][option_code]["summary"]["distance"] / 1000
                    )
                # 케이스 2: result가 'result' 키 하위에 존재하는 경우
                elif (
                    "result" in result
                    and "route" in result["result"]
                    and option_code in result["result"]["route"]
                ):
                    distance = (
                        result["result"]["route"][option_code]["summary"]["distance"]
                        / 1000
                    )
                else:
                    print(f"옵션 {option_code}에 해당하는 정보가 응답에 없습니다.")
                    continue
                ...
                print(f"HTTP 에러 {response.status_code} 발생 (옵션: {option_code})")
