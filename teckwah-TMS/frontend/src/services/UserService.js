...
const UserService = {
  ...
  getUsers: async (params = {}) => {
    try {
      ...
      logger.api('GET', '/users', defaultParams);
      const response = await api.get('/users', { params: defaultParams });
      
      logger.apiResponse('/users', 'success');
  ...
  deleteUser: async (userId) => {
    try {
      ...
      logger.api('DELETE', `/users/${userId}`);
      
      const response = await api.delete(`/users/${userId}`);
      
      logger.apiResponse(`/users/${userId}`, 'success');
      return response.data;
    } catch (error) {
