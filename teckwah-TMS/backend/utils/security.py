...
def get_session(session_id: Optional[str]) -> Optional[Dict[str, Any]]:
    ...
    # 세션 갱신 (만료 시간 연장)
    if session_data:
        session_data["expires_at"] = current_time + (settings.SESSION_EXPIRE_HOURS * 3600)
...
def get_admin_user(user_data: Dict[str, Any] = Depends(get_current_user)) -> Dict[str, Any]:
    """
    현재 사용자가 관리자인지 확인합니다.
