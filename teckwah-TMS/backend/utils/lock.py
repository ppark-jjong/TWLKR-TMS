...
def acquire_lock(
    ...
    try:
        # 락 정보 조회 쿼리 생성
        query = f"""
        SELECT is_locked, updated_by, update_at 
        FROM {table_name} 
        ...
        if not result:
            logger.warning(f"락 획득 실패: {table_name} 테이블에서 ID {row_id}인 행을 찾을 수 없음")
        ...
        if is_locked and locked_by != user_id:
            ...
            if locked_at and locked_at < lock_timeout:
                ...
                logger.info(f"만료된 락 해제: {table_name} ID {row_id}, 이전 사용자: {locked_by}")
                return _update_lock(db, table_name, row_id, user_id, True)
            
            # 락 획득 실패, 현재 락 보유자 정보 반환
            logger.warning(f"락 획득 실패: {table_name} ID {row_id}는 현재 {locked_by}가 락 보유 중")
            return False, {
...
def release_lock(
    ...
    try:
        ...
        if not result:
            logger.warning(f"락 해제 실패: {table_name} 테이블에서 ID {row_id}인 행을 찾을 수 없음")
            return False, {"message": "지정된 항목을 찾을 수 없습니다"}
        ...
        if locked_by != user_id:
            logger.warning(f"락 해제 실패: {table_name} ID {row_id}의 락은 {locked_by}의 소유")
            return False, {
...
def check_lock_status(
    ...
    try:
        ...
        if not result:
            logger.warning(f"락 상태 확인 실패: {table_name} 테이블에서 ID {row_id}인 행을 찾을 수 없음")
            return {
                "editable": False,
...
def _update_lock(
    ...
    try:
        ...
        result = {
            "message": f"락 {action} 성공",
            "editable": lock_status
        }
        
        if lock_status:
            result.update({
                "locked_by": user_id,
                "locked_at": update_data["update_at"]
            })
