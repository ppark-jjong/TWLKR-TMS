...
async def update_order(
    ...
    try:
        ...
        # 주문 수정
        updated_order = DashboardService.update_order(db, dashboard_id, order_data, user_id)
...
async def delete_order(
    ...
    try:
        ...
        # 주문 삭제
        success = DashboardService.delete_order(db, dashboard_id, user_id)
...
async def delete_multiple_orders(
    ...
    try:
        ...
        logger.error(f"주문 일괄 삭제 중 오류: {str(e)}")
        return ErrorResponse(
            success=False,
...
@router.post("/{dashboard_id}/status", response_model=SuccessResponse)
async def update_order_status(
    ...
    try:
        ...
        if user_role != "ADMIN":
            # 현재 주문 상태 확인
            current_order = DashboardService.get_order(db, dashboard_id, user_id)
            ...
            # 상태 역행 체크
            if (
                (current_status in ['COMPLETE', 'ISSUE', 'CANCEL'] and new_status in ['WAITING', 'IN_PROGRESS']) or
                (current_status == 'IN_PROGRESS' and new_status == 'WAITING')
            ):
                return ErrorResponse(
                    success=False,
        ...
        # 주문 상태 변경
        updated_order = DashboardService.update_order_status(
            db, dashboard_id, status_data.status, user_id
        )
...
async def update_multiple_order_status(
    ...
    try:
        ...
        # 일반 사용자의 상태 역행 방지 (컨트롤러에서는 체크하지 않고 서비스 레이어에서 처리)
        ...
        logger.error(f"주문 상태 일괄 변경 중 오류: {str(e)}")
        return ErrorResponse(
            success=False,
            message="주문 상태 일괄 변경 중 오류가 발생했습니다"
        ).model_dump()
...
async def assign_driver_to_orders(
    ...
    try:
        ...
        success_count, failed_count = DashboardService.assign_driver_to_multiple_orders(
            db, driver_data.order_ids, driver_data.driver_name, driver_data.driver_contact, user_id
        )
