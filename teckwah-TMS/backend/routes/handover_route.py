...
from backend.schemas.dashboard_schema import LockResponse
...
async def get_handover(
    handover_id: int = Path(...), request: Request = None, db: Session = Depends(get_db)
):
    """
    인수인계 상세 정보 조회
    - handover_id: 인수인계 ID
    ...
    try:
        ...
        if not handover:
            return ErrorResponse(
                success=False, message="인수인계를 찾을 수 없습니다"
            ).model_dump()

        # 응답 데이터
        return SuccessResponse(
            success=True, message="인수인계 조회 성공", data=handover
        ).model_dump()
        ...
        return ErrorResponse(
            success=False, message="인수인계 조회 중 오류가 발생했습니다"
        ).model_dump()
...
async def get_handovers(
    ...
    try:
        ...
        handovers, total, notices = HandoverService.get_handovers(
            db, skip=skip, limit=limit, is_notice=is_notice
        )
...
async def create_handover(
    ...
    try:
        ...
        if handover_data.is_notice and user_role != "ADMIN":
            return ErrorResponse(
                success=False, message="공지사항 등록 권한이 없습니다"
            ).model_dump()
        ...
        return ErrorResponse(
            success=False, message=f"인수인계 생성 중 오류가 발생했습니다: {str(e)}"
        ).model_dump()
...
async def update_handover(
    ...
    try:
        ...
        if user_role != "ADMIN" and current_handover.get("update_by") != user_id:
            return ErrorResponse(
                success=False, message="이 인수인계를 수정할 권한이 없습니다"
            ).model_dump()

        # 일반 사용자가 공지사항으로 변경하려는 경우 권한 체크
        if (
            handover_data.is_notice is True
            and user_role != "ADMIN"
            and not current_handover.get("is_notice")
        ):
            return ErrorResponse(
                success=False, message="공지사항 등록 권한이 없습니다"
            ).model_dump()
        ...
        # 인수인계 수정
        updated_handover = HandoverService.update_handover(
            db, handover_id, handover_data, user_id
        )
        ...
        return ErrorResponse(
            success=False, message="인수인계 수정 중 오류가 발생했습니다"
        ).model_dump()
...
async def delete_handover(
    ...
    try:
        ...
        # 응답 데이터
        return SuccessResponse(success=True, message="인수인계 삭제 성공").model_dump()
        ...
        return ErrorResponse(
            success=False, message="인수인계 삭제 중 오류가 발생했습니다"
        ).model_dump()
...
async def lock_handover(
    ...
    try:
        ...
        if user_role != "ADMIN":
            current_handover = HandoverService.get_handover(db, handover_id, user_id)
            if not current_handover:
                return {"success": False, "message": "인수인계를 찾을 수 없습니다"}

            if current_handover.get("update_by") != user_id:
                return {
                    "success": False,
                    "message": "이 인수인계를 수정할 권한이 없습니다",
                    "lockStatus": {"editable": False},
                }
        ...
        return {"success": False, "message": "인수인계 락 획득 중 오류가 발생했습니다"}
...
async def unlock_handover(
    ...
    try:
        ...
        return ErrorResponse(
            success=False, message="인수인계 락 해제 중 오류가 발생했습니다"
        ).model_dump()
