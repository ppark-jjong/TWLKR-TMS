  ...
  try {
    ...
    try {
      ...
      if (req.user.user_role !== 'ADMIN' && status !== dashboard.status) {
        const validStatusTransitions = STATUS_TRANSITIONS[dashboard.status] || [];
      ...
      if (dashboard.status === 'WAITING' && status === 'IN_PROGRESS' && !dashboard.depart_time) {
    ...
    return res.status(200).json(
      createResponse(
        ...
        `${results.success.length}건의 주문에 배차 정보가 업데이트되었습니다${
          results.failed.length > 0 ? `, ${results.failed.length}건은 실패했습니다` : ''
        }`,
        {
  ...
  try {
    ...
    switch (chart_type) {
      ...
      case 'department':
        ...
        deptData['CS'] = {};
        csData.forEach(item => {
          deptData['CS'][item.dataValues.status] = parseInt(item.dataValues.count);
        });
        ...
        hesData.forEach(item => {
          deptData['HES'][item.dataValues.status] = parseInt(item.dataValues.count);
        });
        ...
        lenovoData.forEach(item => {
          deptData['LENOVO'][item.dataValues.status] = parseInt(item.dataValues.count);
        });
