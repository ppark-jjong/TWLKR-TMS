 ...
 * 대시보드 목록 조회 API
 * GET /dashboard/list
 * 쿼리 파라미터: start, end, status, department, page, size, sort_by, sort_order
 */
router.get('/list', authenticate, async (req, res, next) => {
  try {
    console.log('대시보드 목록 조회 요청:', req.query);

    // 필수 파라미터 검증
    if (!req.query.start || !req.query.end) {
      console.warn('필수 파라미터 누락: start/end', req.query);
      return res
        .status(400)
        .json(
          createResponse(
            false,
            '조회 기간(start, end)은 필수 파라미터입니다',
            null,
...
/**
 * 대시보드 검색 API - 분리된 검색 기능
 * GET /dashboard/search
 * 쿼리 파라미터: search (필수), page, limit, sort_by, sort_order
 */
router.get('/search', authenticate, async (req, res, next) => {
  try {
    console.log('대시보드 검색 요청:', req.query);

    // 필수 파라미터 검증
    if (!req.query.search) {
      console.warn('필수 파라미터 누락: search', req.query);
      return res
        .status(400)
        .json(
          createResponse(
            false,
            '검색어(search)는 필수 파라미터입니다',
            null,
            ERROR_CODES.VALIDATION_ERROR
          )
        );
    ...
    return res
      .status(200)
      .json(createResponse(true, '대시보드 검색 성공', responseData));
  } catch (error) {
    next(error);
  }
});
