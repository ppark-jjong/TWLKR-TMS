...
async def lifespan(app: FastAPI):
    ...
    logger.info(f"환경: {'개발' if settings.DEBUG else '운영'} 모드")
    logger.info(f"서버 정보: {platform.system()} {platform.release()} ({platform.machine()})")
    logger.info(f"Python 버전: {sys.version}")
    logger.info(f"FastAPI 버전: {fastapi.__version__}")
    
    # 시스템 자원 정보
    memory = psutil.virtual_memory()
    logger.info(f"메모리: 총 {memory.total/1024/1024:.1f} MB, 사용 가능 {memory.available/1024/1024:.1f} MB")
    
    # 데이터베이스 연결 정보
    logger.info(f"데이터베이스: MySQL {settings.MYSQL_HOST}:{settings.MYSQL_PORT}")
    logger.info(f"CORS 설정: {settings.ALLOWED_ORIGINS}")
...
async def spa_middleware(request: Request, call_next):
    ...
    api_routes = [
        "/auth", "/dashboard", "/handover", "/visualization", 
        "/users", "/postal-codes", "/health", "/docs", "/redoc", "/openapi.json"
    ]
    
    # 인증 예외 경로 (로그인 페이지, 인증 API 등)
    auth_exempt_routes = [
        "/login", "/auth/login", "/logo.png", "/favicon.ico",
        "/assets", "/health", "/docs", "/redoc", "/openapi.json"
    ]
    ...
    # 정적 파일 요청인지 확인
    is_static_request = path.startswith('/assets/') or any(path.endswith(ext) for ext in ['.js', '.css', '.png', '.jpg', '.svg', '.ico'])
    ...
    if is_api_request and not is_auth_exempt and not path.startswith("/auth/login"):
        ...
        if not session:
            logger.warning(f"인증되지 않은 API 접근 (세션 만료): {path}, 세션ID: {session_id[:8]}")
            return Response(
                content=json.dumps({
                    "success": False, 
        ...
        log_level(f"인증 성공: 사용자={session['user_id']}, 권한={session['user_role']}")
    ...
    try:
        ...
        if response.status_code == 404:
            ...
            if os.path.exists(index_path):
                ...
                logger.warning(f"SPA 폴백 실패: index.html을 찾을 수 없음 ({index_path})")
