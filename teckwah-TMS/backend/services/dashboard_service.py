...
from backend.schemas.dashboard import DashboardCreate, DashboardUpdate, OrderStatusUpdate, StatusCount
...
class DashboardService:
    ...
    @staticmethod
    def get_order(db: Session, dashboard_id: int, user_id: str) -> Optional[Dict[str, Any]]:
        """주문 상세 정보를 조회합니다."""
        order = db.query(Dashboard).filter(Dashboard.dashboard_id == dashboard_id).first()
        ...
        # 락 정보 추가
        lock_status = check_lock_status(db, DashboardService.TABLE_NAME, dashboard_id, user_id)
    ...
    def get_orders(
        ...
        # 동일한 필터 적용
        if start_date and end_date:
            count_query = count_query.filter(Dashboard.eta.between(start_date, end_date))
        elif start_date:
    ...
    @staticmethod
    def create_order(db: Session, order_data: DashboardCreate, user_id: str) -> Dashboard:
        """새 주문을 생성합니다."""
        try:
            ...
            logger.info(f"새 주문 생성 완료: {order_data.order_no} (ID: {new_order.dashboard_id})")
            return new_order
    ...
    def update_order(
        ...
        """주문 정보를 수정합니다."""
        # 락 확인
        lock_info = check_lock_status(db, DashboardService.TABLE_NAME, dashboard_id, user_id)
        if not lock_info.get("editable", False):
            logger.warning(f"락 없이 주문 수정 시도: ID {dashboard_id}, 사용자 {user_id}")
            raise ValueError(lock_info.get("message", "이 주문을 수정할 권한이 없습니다"))
        ...
        try:
            ...
            updatable_fields = [
                ...
                'postal_code', 'address', 'customer', 'contact',
                'driver_name', 'driver_contact', 'remark'
            ]
            ...
            for field in updatable_fields:
                if hasattr(order_data, field) and getattr(order_data, field) is not None:
            ...
            if new_status and old_status != new_status:
                ...
                if old_status == 'WAITING' and new_status == 'IN_PROGRESS':
                    ...
                    logger.info(f"주문 상태 변경: {old_status} → {new_status}, 출발 시간 기록: {now}")
                
                # 진행 → 완료/이슈/취소 시 complete_time 기록
                elif old_status == 'IN_PROGRESS' and new_status in ['COMPLETE', 'ISSUE', 'CANCEL']:
                    order.complete_time = now
                    logger.info(f"주문 상태 변경: {old_status} → {new_status}, 완료 시간 기록: {now}")
    ...
    def delete_order(db: Session, dashboard_id: int, user_id: str) -> bool:
        """주문을 삭제합니다."""
        # 락 획득 시도
        success, lock_info = acquire_lock(db, DashboardService.TABLE_NAME, dashboard_id, user_id)
        if not success:
            logger.warning(f"락 획득 실패로 주문 삭제 불가: ID {dashboard_id}, 사용자 {user_id}")
            raise ValueError(lock_info.get("message", "이 주문을 삭제할 권한이 없습니다"))
    ...
    @staticmethod
    def delete_multiple_orders(db: Session, order_ids: List[int], user_id: str) -> Tuple[int, int]:
        """여러 주문을 일괄 삭제합니다."""
        # 삭제 성공/실패 카운트
        success_count = 0
        failed_count = 0
    ...
    def update_order_status(
        ...
        success, lock_info = acquire_lock(db, DashboardService.TABLE_NAME, dashboard_id, user_id)
        if not success:
            logger.warning(f"락 획득 실패로 주문 상태 변경 불가: ID {dashboard_id}, 사용자 {user_id}")
            raise ValueError(lock_info.get("message", "이 주문의 상태를 변경할 권한이 없습니다"))
        ...
        try:
            ...
            if old_status == 'WAITING' and status == 'IN_PROGRESS':
                order.depart_time = now
                logger.info(f"주문 상태 변경: {old_status} → {status}, 출발 시간 기록: {now}")
            
            # 진행 → 완료/이슈/취소 시 complete_time 기록
            elif old_status == 'IN_PROGRESS' and status in ['COMPLETE', 'ISSUE', 'CANCEL']:
                order.complete_time = now
                logger.info(f"주문 상태 변경: {old_status} → {status}, 완료 시간 기록: {now}")
            
            # 역행 시 로그 남기기 (관리자 권한 필요 - 컨트롤러에서 체크)
            elif (
                (old_status in ['COMPLETE', 'ISSUE', 'CANCEL'] and status in ['WAITING', 'IN_PROGRESS']) or
                (old_status == 'IN_PROGRESS' and status == 'WAITING')
            ):
                logger.warning(f"상태 역행 감지: {old_status} → {status}, 주문 ID {dashboard_id}, 사용자 {user_id}")
            ...
            db.commit()
            db.refresh(order)
            logger.info(f"주문 상태 변경 완료: ID {dashboard_id}, {old_status} → {status}")
            return order
    ...
    def update_multiple_order_status(
        ...
        for order_id in order_ids:
            try:
                ...
                result = DashboardService.update_order_status(db, order_id, status, user_id)
                if result:
                    success_count += 1
                else:
                    failed_count += 1
        ...
        logger.info(f"일괄 상태 변경 결과: 성공 {success_count}건, 실패 {failed_count}건")
        return success_count, failed_count
    ...
    def assign_driver(
        ...
        success, lock_info = acquire_lock(db, DashboardService.TABLE_NAME, dashboard_id, user_id)
        if not success:
            logger.warning(f"락 획득 실패로 기사 배정 불가: ID {dashboard_id}, 사용자 {user_id}")
            raise ValueError(lock_info.get("message", "이 주문에 기사를 배정할 권한이 없습니다"))
    ...
    def assign_driver_to_multiple_orders(
        ...
        logger.info(f"일괄 기사 배정 결과: 성공 {success_count}건, 실패 {failed_count}건")
        return success_count, failed_count
    ...
    def unlock_order(db: Session, dashboard_id: int, user_id: str) -> Dict[str, Any]:
        """주문의 편집 락을 해제합니다."""
        success, lock_info = release_lock(db, DashboardService.TABLE_NAME, dashboard_id, user_id)
