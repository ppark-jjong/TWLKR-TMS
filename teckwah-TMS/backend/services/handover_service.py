...
from backend.models.handover_model import Handover
from backend.schemas.handover_schema import HandoverCreate, HandoverUpdate
...
class HandoverService:
    ...
    def get_handover(
        ...
        # ORM 객체를 딕셔너리로 변환
        handover_dict = {
            c.name: getattr(handover, c.name) for c in handover.__table__.columns
        }

        # 락 정보 추가
        lock_status = check_lock_status(
            db, HandoverService.TABLE_NAME, handover_id, user_id
        )
    ...
    @staticmethod
    def get_handovers(
        db: Session, skip: int = 0, limit: int = 100, is_notice: Optional[bool] = None
        ...
        # 페이지네이션 및 정렬 적용
        handovers = (
            query.order_by(desc(Handover.create_at)).offset(skip).limit(limit).all()
        )
    ...
    @staticmethod
    def create_handover(
        db: Session, handover_data: HandoverCreate, user_id: str
    ) -> Handover:
        """새 인수인계를 생성합니다."""
        try:
            # 현재 시간 설정
            ...
            logger.info(
                f"새 인수인계 생성 완료: {handover_data.title} (ID: {new_handover.handover_id})"
            )
            return new_handover
    ...
    @staticmethod
    def update_handover(
        db: Session, handover_id: int, handover_data: HandoverUpdate, user_id: str
    ) -> Optional[Handover]:
        """인수인계 정보를 수정합니다."""
        # 락 확인
        ...
        if not lock_info.get("editable", False):
            logger.warning(
                f"락 없이 인수인계 수정 시도: ID {handover_id}, 사용자 {user_id}"
            )
            raise ValueError(
                lock_info.get("message", "이 인수인계를 수정할 권한이 없습니다")
            )
        ...
        if handover.update_by != user_id:
            ...
            logger.warning(
                f"본인 작성 글이 아닌 인수인계 수정 시도: ID {handover_id}, 작성자 {handover.update_by}, 수정 시도 사용자 {user_id}"
            )
        ...
        try:
            ...
            for field in updatable_fields:
                if (
                    hasattr(handover_data, field)
                    and getattr(handover_data, field) is not None
                ):
    ...
    def delete_handover(db: Session, handover_id: int, user_id: str) -> bool:
        ...
        if not success:
            logger.warning(
                f"락 획득 실패로 인수인계 삭제 불가: ID {handover_id}, 사용자 {user_id}"
            )
            raise ValueError(
                lock_info.get("message", "이 인수인계를 삭제할 권한이 없습니다")
            )
