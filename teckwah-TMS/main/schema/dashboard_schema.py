...
class DashboardCreate(DashboardBase):
    ...
    def validate_contact(cls, v):
        ...
        if v and not v.startswith('010-'):
            raise ValueError('연락처는 010-XXXX-XXXX 형식이어야 합니다')
        return v
...
class DashboardUpdate(BaseModel):
    ...
    warehouse: Optional[str] = Field(None, description="창고(SEOUL/BUSAN/GWANGJU/DAEJEON)")
...
class DashboardResponse(DashboardBase):
    ...
    status: str = Field(..., description="상태(WAITING/IN_PROGRESS/COMPLETE/ISSUE/CANCEL)")
    create_time: datetime = Field(..., description="생성 시간", alias="createTime")
    depart_time: Optional[datetime] = Field(None, description="출발 시간", alias="departTime")
    complete_time: Optional[datetime] = Field(None, description="완료 시간", alias="completeTime")
    ...
    duration_time: Optional[int] = Field(None, description="소요 시간(분)", alias="durationTime")
    driver_name: Optional[str] = Field(None, description="기사 이름", alias="driverName")
    driver_contact: Optional[str] = Field(None, description="기사 연락처", alias="driverContact")
    updated_by: Optional[str] = Field(None, description="마지막 업데이트 사용자", alias="updatedBy")
    remark: Optional[str] = Field(None, description="비고")
    ...
    # 클라이언트 표시용 추가 필드
    status_label: Optional[str] = Field(None, description="상태 레이블", alias="statusLabel")
    type_label: Optional[str] = Field(None, description="유형 레이블", alias="typeLabel")
...
class DashboardListResponse(BaseModel):
    ...
    data: List[DashboardResponse] = Field(..., description="주문 목록")
...
class StatusChangeRequest(BaseModel):
    """상태 변경 요청 스키마"""
    ids: List[int] = Field(..., description="변경할 주문 ID 목록")
    status: str = Field(..., description="변경할 상태(WAITING/IN_PROGRESS/COMPLETE/ISSUE/CANCEL)")
...
class DriverAssignRequest(BaseModel):
    """기사 배정 요청 스키마"""
    ids: List[int] = Field(..., description="배정할 주문 ID 목록")
...
class DashboardDeleteRequest(BaseModel):
    """주문 삭제 요청 스키마"""
    ids: List[int] = Field(..., description="삭제할 주문 ID 목록")
...
class LockStatusResponse(BaseModel):
    ...
    locked_at: Optional[datetime] = Field(None, description="락 획득 시간", alias="lockedAt")
