...
const { findWithRowLock, updateWithLock, NotFoundException } = require('../utils/lock-manager');
...
router.get('/:id', authenticate, async (req, res, next) => {
  try {
    ...
    if (!handover.is_notice && req.user.role !== 'ADMIN' && 
        handover.department && handover.department !== req.user.department) {
...
router.post('/', authenticate, async (req, res, next) => {
  try {
    ...
    // 부서 지정 유효성 검사
    if (department && req.user.role !== 'ADMIN' && department !== req.user.department) {
      return res.status(403).json(createResponse(
        false,
...
router.put('/:id', authenticate, async (req, res, next) => {
  try {
    ...
    if (!title && !content && is_notice === undefined && 
        !department && !priority && expiry_date === undefined) {
      return res.status(400).json(createResponse(
        false,
        '수정할 내용을 입력해주세요',
        null,
    ...
    try {
      ...
      const updatedHandover = await updateWithLock(Handover, id, updateData, userId);
